// enhanced_static_video_template.js - Fixed visual function handling
...
class VideoGenerator {
  constructor(dynamicContent, options = {}) {
  ...
  setupPaths() {
    const inputPath = this.options.inputPath;
    const videoName = this.options.videoName;
    
    // Calculate output path based on input structure
    if (inputPath) {
      const relativePath = path.relative('input', path.dirname(inputPath));
      const baseName = path.basename(inputPath, '.md');
      const finalVideoName = videoName || baseName;
      
      this.OUTPUT_DIR = path.join('output', relativePath, finalVideoName);
    } else {
      this.OUTPUT_DIR = path.join('output', videoName || `video_${Date.now()}`);
    }
    ...
    // Use the final video name for the output file
    const finalVideoName = videoName || (inputPath ? path.basename(inputPath, '.md') : `video_${Date.now()}`);
    this.FINAL_VIDEO = path.join(this.OUTPUT_DIR, `${finalVideoName}.mp4`);
  }
  ...
  processContent() {
    // Merge dynamic speakers with defaults
    this.speakers = { ...STATIC_CONFIG.DEFAULT_SPEAKERS, ...this.dynamicContent.speakers };
    
    // Process lesson steps
    this.lessonSteps = this.dynamicContent.lessonSteps.map(step => ({
      ...step,
    ...
    // Process visual functions - Fix string functions
    this.visualFunctions = this.processVisualFunctions(this.dynamicContent.visualFunctions || {});
  }

  // NEW: Process visual functions and convert strings to actual functions
  processVisualFunctions(visualFunctions) {
    const processedFunctions = {};
    
    Object.keys(visualFunctions).forEach(functionName => {
      const funcCode = visualFunctions[functionName];
      
      if (typeof funcCode === 'string') {
        try {
          // Convert string function to actual function
          console.log(`🔧 Converting visual function '${functionName}' from string to function`);
          
          // Check if it's a function declaration or expression
          let actualFunction;
          if (funcCode.trim().startsWith('function')) {
            // Function declaration: "function name() { ... }"
            actualFunction = eval(`(${funcCode})`);
          } else {
            // Function expression or arrow function
            actualFunction = eval(funcCode);
          }
          
          processedFunctions[functionName] = actualFunction;
          console.log(`✅ Successfully converted visual function '${functionName}'`);
          
        } catch (error) {
          console.warn(`⚠️ Failed to parse visual function '${functionName}':`, error.message);
          console.warn(`Function code preview: ${funcCode.substring(0, 100)}...`);
          
          // Create a placeholder function that shows error
          processedFunctions[functionName] = (ctx, ...args) => {
            ctx.save();
            ctx.fillStyle = '#ff6b6b';
            ctx.font = '16px Arial';
            ctx.textAlign = 'center';
            ctx.fillText(`Error in visual function: ${functionName}`, 500, 350);
            ctx.fillText('Check console for details', 500, 370);
            ctx.restore();
          };
        }
      } else if (typeof funcCode === 'function') {
        // Already a function
        processedFunctions[functionName] = funcCode;
        console.log(`✅ Visual function '${functionName}' is already a function`);
      } else {
        console.warn(`⚠️ Visual function '${functionName}' is neither string nor function:`, typeof funcCode);
      }
    });
    
    console.log(`📊 Processed ${Object.keys(processedFunctions).length} visual functions`);
  ...
  async combineVideoWithTimedAudio(videoPath, lessonSteps, outputPath) {
    return new Promise((resolve, reject) => {
      console.log(`🎵 Combining video with timed audio tracks...`);
      ...
      lessonSteps.forEach((step, i) => {
        if (step.audioPath && step.actualAudioDuration > 0) {
      ...
      const audioTracks = lessonSteps
        .map((step, i) => step.audioPath && step.actualAudioDuration > 0 ? `[a${i}]` : null)
        .filter(Boolean);
      
      let mixFilter = '';
      if (audioTracks.length > 0) {
        mixFilter = `${audioTracks.join('')}amix=inputs=${audioTracks.length}:duration=longest[audioout]`;
  ...
  // STATIC DRAWING FUNCTIONS - These never change
  ...
  // ENHANCED: Dynamic visual functions with better error handling
  callVisualFunction(ctx, functionName, ...args) {
    if (this.visualFunctions[functionName] && typeof this.visualFunctions[functionName] === 'function') {
      ...
      ctx.fillText('Check console for details', this.config.LAYOUT.MEDIA.x + this.config.LAYOUT.MEDIA.width / 2, this.config.LAYOUT.MEDIA.y + this.config.LAYOUT.MEDIA.height / 2 + 25);
      ctx.restore();
    }
  }
  ...
  async generateFrame(frameGlobalIndex, stepData) {
    ...
    // Background
    const backgroundColor = this.config.SPEAKER_BACKGROUNDS[stepData.speaker] || this.config.COLORS.background;
    ...
    ctx.textAlign = 'center';
    ctx.fillText(stepData.title, this.config.LAYOUT.TITLE.x, this.config.LAYOUT.TITLE.y);
    
    // Content
    ctx.fillStyle = this.config.COLORS.text;
    if (stepData.content) {
      ctx.font = '22px Arial';
      this.wrapText(ctx, stepData.content, this.config.LAYOUT.CONTENT.x, this.config.LAYOUT.CONTENT.y, this.config.LAYOUT.CONTENT.width, 30);
    }
    if (stepData.content2) {
      ctx.font = '22px Arial';
      this.wrapText(ctx, stepData.content2, this.config.LAYOUT.CONTENT2.x, this.config.LAYOUT.CONTENT2.y + (stepData.content ? 30 : 0), this.config.LAYOUT.CONTENT2.width, 26);
    }

    // Draw media area border for visual context
  ...
  async generate() {
    try {
      ...
      let totalFramesToGenerate = Math.ceil(targetVideoDuration * this.config.FPS);
...
// ✅ FIXED: Added the missing module export
